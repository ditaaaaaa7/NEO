...
if (empty($_SESSION['username'])) {
    ...
    <body class="hold-transition sidebar-mini">
        ...
        <div class="wrapper">
            ...
            $userType = "Admin"; // Contoh tipe pengguna
            $username = "admin"; // Contoh nama pengguna
        
            include "navigation.php";

            if ($userType == "Admin" && $username == "admin") {
                include "sidebar.php"; // Sidebar untuk admin
            }
            ?>
                ...
                <section class="content-header">
                    <div class="card card-outline card-danger">
                            ...
                            <div class="container-fluid">
                                <form method="post" enctype="multipart/form-data" action="">
                                    <div class="row">
                                        ...
                                        $query = "SELECT MAX(po_code) AS max_code FROM pr_po_list";
                                        $result = mysqli_query($conn, $query);
                                        $data = mysqli_fetch_array($result);
                                        $code = $data['max_code'];

                                        $urutan = (int) substr($code, 3, 4);
                                        $urutan++;
                                        $huruf = "PR";
                                        ...
                                        <tbody>
                                            <tr class="pr-item">
                                                ...
                                                <td class="text-right total-price">
                                                    <input type="number" name="total_price" class="form-control total-price"
                                    ...
                                    <div class="row justify-content-center">
                                        ...
                                        <a class="btn btn-md btn-secondary" href="purchase_request.php">Cancel</a>
                                    </div>
                                </form>
                            </div>
                        </div>
                        ...
                        if (isset($_POST['submit'])) {
                            ...
                            if ($result) {
                                $id = mysqli_insert_id($conn); // Dapatkan ID yang baru saja dimasukkan
                    
                                foreach ($conn as $item) {
                                    $qty = $item['qty'];
                                    $item_id = $item['item']; // ID item yang Anda ambil dari formulir.
                                    $unit = $item['unit'];
                                    $cost = $item['cost'];
                                    $total_price = $item['total_price'];

                                    $itemDetailQuery = "INSERT INTO item_detail (id, qty, item_id, unit, cost, total_price) VALUES ('$id', '$qty', '$item_id', '$unit', '$cost', '$total_price')";

                                    $itemDetailResult = mysqli_query($item, $itemDetailQuery);

                                    if (!$itemDetailResult) {
                                        echo "Gagal menyimpan item-detail: " . mysqli_error($conn);
                                    }
                                }

                                echo "Data telah disimpan.";
                            } else {
                                echo "Gagal menyimpan data pada tabel pertama: " . mysqli_error($conn);
                            }
